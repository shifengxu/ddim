...
class ScheduleBatch:
    ...
    def schedule_single(self, f_path, lr, lp, order=None, skip_type=None):
        ...
        m_arr2 = [f"order from param    : {order}",
                  ...
                  f"skip_type from param: {skip_type}",
                  f"skip_type from file : {skip_type_}",
                  f"skip_type final     : {skip_type or skip_type_}"]
        order = order or order_
        skip_type = skip_type or skip_type_
        ...
        _, idx_arr = self.vs(torch.tensor(alpha_bar, device=args.device), include_index=True)
        ...
        scheduled_file = self.train(alpha_bar, order, skip_type, new_arr, f_name)
            ...
            if line.startswith('#'):
                ...
                if key.strip() == 'order':
                    if order is None:
                        ...
                        raise ValueError(f"duplicate order in {f_path}")
                if key.strip() == 'skip_type':
    ...
    def train(self, alpha_bar, order, skip_type, m_arr, f_name):
        args = self.args
        if args.lp >= 0:
            lp = args.lp
        else:
            lp = 1.0 / len(alpha_bar)
            if skip_type == 'time_quadratic':
                lp /= 2
            log_fn(f"new lp: {lp:.6f}")
        model = self.model_generate(alpha_bar, lp, args.device)
