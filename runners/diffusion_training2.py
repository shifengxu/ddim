...
class DiffusionTraining2(Diffusion):
    ...
    def train(self):
        ...
        if self.ema_flag:
            self.ema_et = EMAHelper(mu=self.ema_rate, update_every=self.ema_updy)
    ...
    def train_model(self, x, epsilon, epoch):
        ...
        if self.ema_flag and epoch >= self.ema_start_epoch:
            ema_update_flag = self.ema_et.update_ema(self.model_et)
            self.ema_x0.update_ema(self.model_x0)
