"""
Find beta schedule.
Different mode may need different arguments. Here is an sample for
different value of accu_var
=========================================
 accu_var=True
 lr=0.000001
 aacum0_lambda=10000000
 -----------------------------------------
 accu_var=False
 lr=0.2
 aacum0_lambda=100
...
def parse_args():
    ...
    parser.add_argument('--lr', type=float, default=0.2, help='accu_var ? 0.000001 : 0.2')
    ...
    parser.add_argument("--accu_var", type=utils.str2bool, default=True, help='accumulate variance')
    parser.add_argument("--aacum0_lambda", type=float, default=100, help='accu_var ? 10000000 : 100')
...
def accumulate_variance(args, coefficient, alpha):
    """
    accumulate variance from x_1000 to x_1.
    Standard deviation (sigma) is just the coefficient. So variance will be its square.
    let var_i = sigma_i ** 2, such as var_3 = sigma_3 * sigma_3.
    """
    coef_sq = coefficient ** 2
    coef_sq = torch.flip(coef_sq, dims=(0,))    # [var_999, var_998, ,,, var_0]
    mu_sq = torch.reciprocal(alpha)             # mu square
    mu_sq_cp = torch.cumprod(mu_sq, dim=0)      # cum-product
    mu_sq_cp = torch.flip(mu_sq_cp, dims=(0,))  # [ms_999, ms_998, ,,, ms_0]
    t_one = torch.ones((1,), dtype=torch.float64, device=args.device)
    mu_sq_cp2 = torch.cat([mu_sq_cp[1:], t_one], dim=0)  # [ms_998, ms_997, ,,, ms_0, 1]
    prod = torch.mul(coef_sq, mu_sq_cp2)
    res = torch.sum(prod)
    return res
...
def main():
    ...
    log_fn(f"accu_var: {args.accu_var}")
    ...
    for e_idx in range(e_start, e_cnt):
        ...
        if args.accu_var:
            loss_var = accumulate_variance(args, coefficient, alpha)
        else:
            loss_var = torch.var(coefficient)
        loss_min = torch.square(aacum0 - args.aa_low) * args.aacum0_lambda
